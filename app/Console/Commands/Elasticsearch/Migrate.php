...
class Migrate extends Command
{
     ...
     * @var string
     */
    protected $description = 'ES 索引结构迁移';
    ...
    {
        foreach ($this->indices as $indexClass) {
            ...
            if (!$this->es->indices()->exists(['index' => $aliasName])) {
                ...
                $this->info($aliasName . '索引不存在，准备创建');
                ...
                $this->info('索引已存在');
                $option = $this->ask('是否重建索引, yes or no');
                if ($option == 'yes') {
                    $this->es->indices()->delete(['index' => $aliasName.'_0']);
                    $this->info('删除索引成功');
                    $this->handle();
                }
            }
